...
const SeekerMessages = () => {
  ...
  const fetchConversations = async () => {
    try {
      ...
      if (userId) {
        const conversation = response.data.conversations.find(conv => conv.user._id === userId);
        if (conversation) {
          setSelectedConversation(conversation);
          fetchMessages(conversation.user._id);
        }
      } else if (response.data.conversations.length > 0 && !selectedConversation) {

        setSelectedConversation(response.data.conversations[0]);
        fetchMessages(response.data.conversations[0].user._id);
  ...
  useEffect(() => {
    if (userId && (!selectedConversation || selectedConversation.user._id !== userId)) {
  ...
  const handleConversationSelect = (conversation) => {
    ...
    navigate(`/seeker/messages/${conversation.user._id}`);
  };
    ...
    <div className="h-full flex flex-col">
      <div className="bg-white shadow rounded-lg px-4 py-5 sm:p-6 mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Communicate with donors and doctors regarding donation requests.
        </p>
      </div>
